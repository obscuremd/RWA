    ...
    return (
      <Link to={link}>
        <motion.button
          ...
          whileHover={{
            borderColor: "#445B8A",
            boxShadow: "0 0 5px rgba(68, 91, 138, 0.25)",
          }}
        ...
        >
          <div className="p-1 rounded-full border-[1px] border-[#445B8A] inline-flex">
            {icon}
          </div>
          <p>{name}</p>
        </motion.button>
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="flex justify-center items-center gap-3">
          {user && (
            <img src={user.imageUrl} alt="" className="w-12 rounded-full" />
          )}
          <div>
            {user && (
              <p
                style={{ fontSize: Shared.Text.large }}
                className="font-bold capitalize max-w-[8em] truncate"
              >
                {user.username}
              </p>
            )}
            {user && (
              <p
                ...
                className="opacity-50 capitalize max-w-[8em] truncate"
              >
        ...
        <div className="p-3 border-[1px] border-[#445B8A] rounded-3xl flex flex-col gap-4">
          ...
          <p style={{ fontSize: Shared.Text.small }}>
            ...
            <br /> get the latest updates
          </p>
          <div
            style={{ fontSize: Shared.Text.small }}
            className="p-2 rounded-full border-[1px] border-[#445B8A] text-center text-nowrap"
          >
            Subscribe Now
          </div>
        </div>
